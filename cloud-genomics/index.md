...
layout: lesson
root: .
title: Data Carpentry: Connecting to the cloud in 5 min or less
minutes: 5
---

This lesson is based on the Data Carpentry Cloud Genomics lesson, with mothur
details included 
...
To save time, your instructor will have launched an remote computer (instance) for you prior to the workshop. If you are following these lessons on your own, or after the workshop see the lesson on [launching the mothur AMI on your own](http://mothur.org/wiki/Mothur_AMI) for instructions on how to do this yourself.  The logon information will be:
...
```bash
mothur@ip-172-31-62-209 ~ $
```

Test it out with a few commands
...
Now that we have connected, there are a few commands that tell you a little about the machine you have connected to:

- `whoami` - shows your username on computer you have connected to:

```bash
mothur@ip-172-31-62-209 ~ $ whoami
mothur
```
-  `df -h` - shows space on hard drive* 

```bash
mothur@ip-172-31-62-209 ~ $ df -h
...
```bash
mothur@ip-172-31-62-209 ~ $ cat /proc/cpuinfo
processor  : 0
